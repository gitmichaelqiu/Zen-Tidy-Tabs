...
(() => {
  const CONFIG = {
    ...
    MIN_TABS_FOR_SORT: 1, // Show the button if there is at least one tab to sort folder
    DEBOUNCE_DELAY: 250,
  ...
  const findFolderElement = (topicName, workspaceId) => {
    ...
    try {
      ...
      if (!separator.querySelector("#sort-button")) {
        ...
        const buttonFragment = window.MozXULElement.parseXULToFragment(`
                    ...
                    `);
        const buttonNode = buttonFragment.querySelector("toolbarbutton").cloneNode(true);
    ...
    batchDOMUpdates([
      () => {
        separators.forEach((separator) => {
          ...
          if (tidyButton) {
            // Show button if we have any ungrouped tabs
            const shouldShowTidyButton = ungroupedTotal >= CONFIG.MIN_TABS_FOR_SORT; 
